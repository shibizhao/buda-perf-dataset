...
from benchmark.common.benchmark_compile import BenchmarkCompile
...
os.environ["PYBUDA_BUILD_DIR"] = "./tt_build/"
...
def compile(
    ...
    eval_fn: Callable,
    benchmark_compile: BenchmarkCompile,
) -> Dict[str, Any]:
    ...
    benchmark_compile.set_input_shape(input_data, model)
    ...
    if args.device == "tt":
        ...
        def data_parallel_tti_run(
            ...
            if isinstance(model, dict):
                if benchmark_compile.has_compile_inputs:
                    sample_inputs = model["compile_inputs"]

            if benchmark_compile.has_forward_wrapper:
            ...
            if tt_device is not None:
                ...
                monitor_thread = threading.Thread(target=benchmark_compile.cpu_usage_monitor)
                monitor_thread.start()
                benchmark_compile.start_compilation_timer()
                tt_device.compile_to_image(img_path=image_path, training=False, sample_inputs=single_device_inputs)
                benchmark_compile.stop_monitoring = True
                benchmark_compile.end_compilation_timer()
                monitor_thread.join()
            ...
            benchmark_compile.start_benchmark_timer()
            ...
            benchmark_compile.end_benchmark_timer()
            runner.shutdown()
        ...
        os.environ["PYBUDA_OVERRIDE_CLUSTER_YAML"] = "/root/cluster_desc.yaml"
        os.environ["PYBUDA_OVERRIDE_DEVICE_YAML"] = "/root/wormhole_b0_2050_0x0.yaml"

        pybuda.set_configuration_options(backend_runtime_params_path="/root/wh_n300_syslevel.yaml")
        ...
        if(len(device_list) == 0):
            # If no devices are found, set default to Wormhole_B0
            device_list = [BackendDevice.Wormhole_B0]
        ...
        if not args.parallel_tti:
            if isinstance(model, dict):
                ...
                benchmark_compile.end_compilation_timer()
                monitor_thread.join()
                if not benchmark_compile.has_forward_wrapper:
    ...
    if args.device == "tt" and isinstance(model, dict):
        ...
        if pybuda.error_raised():
            ...
            return {
                ...
                "machine_name": benchmark_compile.machine_name,
                "pybuda_hash": benchmark_compile.pybuda_hash,
            }
    else:
    ...
    if args.model_output:
        store_model_output(model, benchmark_compile, store_outputs, store_labels)

    # Benchmark results
    eval_score = eval_fn(outputs=store_outputs, labels=store_labels)
    output_stats_dict = benchmark_compile.calc_output_stats(store_outputs, model, eval_score)
    benchmark_compile.print_output_stats()
...
if __name__ == "__main__":
    ...
    benchmark_compile = BenchmarkCompile(args=args)
    logger.info(f" creating benchmarking run: {func.__name__}")
    logger.info(f" kwargs: {kwargs}")
    model, generator, eval_fn = models[args.model]["func"](benchmark_run=benchmark_compile, **kwargs)
    error = False
    try:
        result = compile(args, model, generator, eval_fn, benchmark_compile)
    ...
    if args.save_output:
        ...
        fname = f"perf_{args.model}_{args.config}_{result.get('input_size', 'na')}_{args.device}_mb{args.microbatch}_compile.json"
