...
def t5_past_cache_enc_dec(training: bool, task: str, config: str, microbatch: int, device: str, data_type: str):
    ...
    if device == "tt":
        ...
        # T5 past cache encoder-decoder overrides
        # Flags
    ...
    if task == "na":
        ...
        # TODO: hangs with 256, hangs with 64
        input_length = 64
        min_new_tokens = 0
    ...
    def preprocessing(input_text, tokenizer=tokenizer, input_length=input_length):
        encoder_inputs = tokenizer(
            input_text, return_tensors="pt", truncation=True, padding="max_length", max_length=input_length
        )
    ...
    if task == "na":
        ...
        dataset = PipelineDataset(
            dataset=sst2_dataset, input_text="sentence", label="label", prepend_text="sst2 sentence: "
        )
        # preprocess
        dataset.data = [(preprocessing(input_text), label) for (input_text, label) in dataset.data]
