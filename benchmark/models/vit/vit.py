...
def vit(training: bool, task: str, config: str, microbatch: int, device: str, data_type: str):
    ...
    if task == "na":
        ...
        if device == "tt":
            model = {"device": pybuda.PyTorchModule(f"pt_vit_{config}", model)}
        else:
    ...
    # image classification task
        ...
        if device == "tt":
            model = {"device": pybuda.PyTorchModule(f"pt_vit_{config}", model)}
            # model = pybuda.PyTorchModule("ViT_classif_16_224", model)
        else:
            model = model.to(device, dtype=torch_df_from_str(data_type))
